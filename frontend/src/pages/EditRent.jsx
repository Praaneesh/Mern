import { updateRent, getRent, getClients, getMaterials } from '../api'
...
export function EditRent() {
    ...
    useEffect(() => {
        async function loadData() {
            ...
            const rentData = await getRent(id)
            if (rentData && clientsData && materialsData) {
                const c = clientsData.find(cl => cl._id === rentData.clientId)
                if (c) {
                    setSelectedClient({
                        value: c._id,
                ...
                if (rentData.dateAdded) setStartDate(new Date(rentData.dateAdded).toISOString().split('T')[0])
                if (rentData.endDate) setEndDate(new Date(rentData.endDate).toISOString().split('T')[0])

                const formattedMaterials = rentData.materials.map(rm => {
                    const liveMat = materialsData.find(m => m._id === rm.materialId)
                    // Prefer the rate saved on the rent line (user may have overridden it), fall back to live material rate
                    const savedRate = parseFloat(rm.weeklyRent ?? rm.price ?? 0)
                    const liveRate = parseFloat(liveMat?.weeklyRent ?? liveMat?.price ?? 0)
                    const weeklyRent = savedRate || liveRate
                    return {
                        material: {
                            value: rm.materialId,
                            label: `${rm.materialName} · Rs.${weeklyRent.toFixed(2)}/wk`,
                            weeklyRent,
                            name: rm.materialName
                        },
                        quantity: rm.quantity,
                        rateInput: String(weeklyRent)
                    }
                })
                setRentMaterials(formattedMaterials.length > 0 ? formattedMaterials : [{ material: null, quantity: 1, rateInput: '' }])
            }
            setIsFetching(false)
        }
        loadData()
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            const msg = err.response?.data?.message || 'Failed to update rent. Please try again.'
    ...
    if (isFetching) {
        return (
            <div className="page-container centered-page animate-in">
                <div style={{ color: 'var(--text-light)', fontSize: '16px' }}>Loading rent data…</div>
            </div>
        )
    }
        ...
        <div className="page-container centered-page animate-in">
            <div style={{ width: '100%', maxWidth: '680px' }}>
                ...
                <form className="form-card" onSubmit={handleSubmit} style={{ maxWidth: '100%' }} ref={formTopRef}>
                    ...
                    <h1 className="form-title" style={{ marginBottom: 0 }}>Edit Rent</h1>
